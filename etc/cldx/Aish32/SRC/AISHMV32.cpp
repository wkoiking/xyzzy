...
/*-------------------------------------------------------------------------*/
int WINAPI
Aishmv(const HWND _hwnd, LPCSTR _szCmdLine, LPSTR _szOutput,
  const DWORD _dwSize)
/* 1. コマンド・ラインでの実行 */
{
	char *argv[4096];
	...
	*Gstdout_buf = '\0';
	...
	ret = aishmv_main(_hwnd, argc, argv);
...
/*-------------------------------------------------------------------------*/
/* 2. バージョンの取得 */
WORD WINAPI AishmvGetVersion(VOID)
{
	return AISHMV_VERSION;
}
/*--------------------------------------------------------------------------*/
/* 3. カーソルの回転速度の取得 */
WORD WINAPI AishmvGetCursorInterval(VOID)
{
	return GStatus.CursorInterval;
}
/*--------------------------------------------------------------------------*/
/* 4. カーソルの回転速度の設定 */
BOOL WINAPI AishmvSetCursorInterval(const WORD _Interval)
{
	BOOL ret;
...
BOOL WINAPI AishmvGetBackGroundMode(VOID)
{
	return FALSE; // 常に非バックグラウンド・モード
}
/*--------------------------------------------------------------------------*/
/* 6. バックグラウンド・モードにするか設定 */
BOOL WINAPI AishmvSetBackGroundMode(const BOOL _bBackGroundMode)
{
	BOOL ret;
...
/*-------------------------------------------------------------------------*/
/* 7. カーソルを表示するモードかを取得 */
BOOL WINAPI AishmvGetCursorMode(VOID)
{
	return GStatus.CursorDisplayMode;
}
/*-------------------------------------------------------------------------*/
/* 8. カーソルを表示するモードかを設定 */
BOOL WINAPI AishmvSetCursorMode(const BOOL _bCursorMode)
{
	BOOL ret;
...
/*-------------------------------------------------------------------------*/
/* 10. DLL が動作中かを返す */
BOOL WINAPI AishmvGetRunning(VOID)
{
	return is_running();
}
/*--------------------------------------------------------------------------*/
...
BOOL WINAPI AishmvCheckArchive(LPCSTR _szFileName, const int _iMode)
{
	BOOL exists;
...
BOOL WINAPI AishmvQueryFunctionList(const int _iFunction)
{
	BOOL ret;

	switch(_iFunction) {
	case ISARC:                      //  1. Aishmv()
	case ISARC_GET_VERSION:          //  2. AishmvGetVersion()
	case ISARC_GET_CURSOR_INTERVAL:  //  3. AishmvGetCursorInterval()
	case ISARC_SET_CURSOR_INTERVAL:  //  4. AishmvSetCursorInterval()
	case ISARC_GET_BACK_GROUND_MODE: //  5. AishmvGetBackGroundMode()
	case ISARC_SET_BACK_GROUND_MODE: //  6. AishmvSetBackGroundMode()
	case ISARC_GET_CURSOR_MODE:      //  7. AishmvGetCursorMode()
	case ISARC_SET_CURSOR_MODE:      //  8. AishmvSetCursorMode()
	case ISARC_GET_RUNNING:          // 10. AishmvGetRunning()
	case ISARC_CHECK_ARCHIVE:        // 16. AishmvCheckArchive()
	case ISARC_QUERY_FUNCTION_LIST:  // 18. AishmvQueryFunctionList()
		ret = TRUE;
		break;
	default:
		ret = FALSE;
